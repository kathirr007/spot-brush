    ...
    <div>
        ...
        <div id="toolbar" style="position: absolute; top: 10px; left: 10px;">
            ...
            <div class="btn-group">
                ...
                <button style="position: relative; display: none;" id="uploadWebDavBtn" title="Save whiteboard to webdav" type="button">
...
<script>
    // import main from '~/pages/whiteboard/js/main'
    ...
    export default {
        ...
        methods: {
            main() {
                this.signaling_socket = io("", { path: this.subdir + "/ws-api" }); // Connect even if we are in a subdir behind a reverse proxy

                this.signaling_socket.on("connect", function() {
                    console.log("Websocket connected!");

                    this.signaling_socket.on("whiteboardConfig", (serverResponse) => {
                        ConfigService.initFromServer(serverResponse);
                        // Inti whiteboard only when we have the config from the server
                        initWhiteboard();
                    });

                    this.signaling_socket.on("whiteboardInfoUpdate", (info) => {
                        InfoService.updateInfoFromServer(info);
                        whiteboard.updateSmallestScreenResolution();
                    });

                    this.signaling_socket.on("drawToWhiteboard", function(content) {
                        whiteboard.handleEventsAndData(content, true);
                        InfoService.incrementNbMessagesReceived();
                    });

                    this.signaling_socket.on("refreshUserBadges", function() {
                        whiteboard.refreshUserBadges();
                    });

                    let accessDenied = false;
                    this.signaling_socket.on("wrongAccessToken", function() {
                        if (!accessDenied) {
                            accessDenied = true;
                    ...
                    this.signaling_socket.emit("joinWhiteboard", {
                        wid: whiteboardId,
                        at: accessToken,
